...
func Part1(fileName string) int {
	...
	var walkNode func(Node, map[string]bool, []string)
	walkNode = func(node Node, visitedNodes map[string]bool, route []string) {
		if node.name == "end" {
		...
		if unicode.IsLower(rune(node.name[0])) {
			visitedNodes[node.name] = true
		}

		route = append(route, node.name)
		for _, connectingNode := range node.connections {
			duplicateRoute := make([]string, len(route))
			copy(duplicateRoute, route)
			walkNode(nodes[connectingNode], duplicateMap(visitedNodes), duplicateRoute)
		}
		return
	}

	walkNode(nodes["start"], make(map[string]bool), []string{})
	return len(routes)
}
...
func parseInputLines(input []string) *map[string]Node {
	...
	createNode := func(nodeName string, connection string) {
		if node, ok := result[nodeName]; ok {
			updatedNode := Node{
				name:        nodeName,
				connections: append(node.connections, connection),
			}
			result[nodeName] = updatedNode
			return
		}
