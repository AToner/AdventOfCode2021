...
sea floor. On a small screen, the sonar sweep report (your puzzle day02.txt) appears: each line is a measurement of the
...
func Part2(inputFile string) int {
	...
	sumOfWindowFrom := func(start int, windowSize int) int {
		result := 0
		for index := start; (index < start+windowSize) && index < maxIndex; index++ {
	...
	for index := 0; index < len(input); index++ {
		sums = append(sums, sumOfWindowFrom(index, 3))
	}
