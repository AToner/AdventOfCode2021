...
func Part1(fileName string) int {
	...
	walkRoute(grid, 0, 0, -grid[0][0].height)
	return grid[lastRow][lastColumn].lowestScore
}

func walkRoute(grid [][]Cell, row int, column int, previousScore int) {
	lastRow := len(grid) - 1
	lastColumn := len(grid[0]) - 1
	...
	walkRoute(grid, row-1, column, score)
	walkRoute(grid, row, column-1, score)
	walkRoute(grid, row+1, column, score)
	walkRoute(grid, row, column+1, score)
}
...
each time the tile repeats to the right or downward, all of its risk levels are 1 higher than the tile immediately up
or left of it. However, risk levels above 9 wrap back around to 1. So, if your original map had some position with a
risk level of 8, then that same position on each of the 25 total tiles would be as follows:
...
func Part2(fileName string) int {
	...
	sourceRows := len(grid)
	sourceColumns := len(grid[0])

	fullGrid := emptyGrid(sourceRows*5, sourceColumns*5)
	for rowIndex, row := range fullGrid {
		for columnIndex, _ := range row {
			sourceRowIndex := rowIndex % sourceRows
			rowMultipleier := rowIndex / sourceRows
			sourceColumnIndex := columnIndex % sourceColumns
			columnMultiplier := columnIndex / sourceColumns
			multiplier := rowMultipleier + columnMultiplier

			fullGrid[rowIndex][columnIndex] = Cell{
				height:      grid[sourceRowIndex][sourceColumnIndex].height + (1 * multiplier),
				lowestScore: math.MaxInt,
			}
			if fullGrid[rowIndex][columnIndex].height >= 10 {
				fullGrid[rowIndex][columnIndex].height = fullGrid[rowIndex][columnIndex].height - 9
			}
		}
	}

	lastRow := len(fullGrid) - 1
	lastColumn := len(fullGrid[0]) - 1
	walkRoute(fullGrid, 0, 0, -fullGrid[0][0].height)
	return fullGrid[lastRow][lastColumn].lowestScore
}
...
func incrementedGrid(grid [][]Cell, addition int) [][]Cell {
	rows := len(grid)
	columns := len(grid[0])
	...
	for rowIndex, row := range grid {
		for columnIndex, _ := range row {
			result[rowIndex][columnIndex] = Cell{
				height:      grid[rowIndex][columnIndex].height + addition,
				lowestScore: math.MaxInt,
			}
			if result[rowIndex][columnIndex].height >= 10 {
				result[rowIndex][columnIndex].height = result[rowIndex][columnIndex].height - 9
			}
		}
	}
