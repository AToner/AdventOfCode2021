...
func upEnergy(grid [][]Octopus) {
	for rowIndex, _ := range grid {
		for columnIndex, _ := range grid[rowIndex] {
			grid[rowIndex][columnIndex].energy += 1
		}
	}
}

func cleanGrid(grid [][]Octopus) {
	for rowIndex, _ := range grid {
		for columnIndex, _ := range grid[rowIndex] {
			grid[rowIndex][columnIndex].flashed = false
			if grid[rowIndex][columnIndex].energy > 9 {
				grid[rowIndex][columnIndex].energy = 0
			}
		}
	}
}
...
func parseInputLines(lines []string) ([][]Octopus, int, int) {
	...
	result := make([][]Octopus, rowCount)
	for row := range result {
		result[row] = make([]Octopus, columnCount)
	}
	...
	for rowIndex, line := range lines {
		...
		for columnIndex, cell := range row {
			...
			result[rowIndex][columnIndex] = Octopus{
				energy:  value,
				flashed: false,
			}
		}
...
func displayGrid(grid [][]Octopus) {
	for rowIndex, _ := range grid {
		rowString := ""
		for columnIndex, _ := range grid[rowIndex] {
			rowString = rowString + strconv.Itoa(grid[rowIndex][columnIndex].energy)
		}
