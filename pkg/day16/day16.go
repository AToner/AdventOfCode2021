...
operator packet can use one of two modes indicated by the bit immediately after the packet header; this is called the length type ID:
...
func Part1(fileName string) int {
	...
	for _, line := range input {
		linePacket := parseCode(line)
		results = append(results, totalVersions(linePacket, 0))
	}

	sum := func(input []int64) int {
		var result int
...
func Part2(fileName string) int {
	var calcPacket func(packet) int64
	calcPacket = func(input packet) int64 {
		if len(input.subPackets) == 0 {
			return input.value
		}
		...
		case sum:
			var result int64
			for _, aPacket := range input.subPackets {
				result += calcPacket(aPacket)
			}
			return result
		case product:
			var result int64
		...
		case minimum:
			...
			for _, aPacket := range input.subPackets {
				aPacketValue := calcPacket(aPacket)
				if aPacketValue < result {
					result = aPacketValue
				}
			}
			return result
		case maximum:
			...
			return result
		case greater:
			if calcPacket(input.subPackets[0]) > calcPacket(input.subPackets[1]) {
				return 1
			}
			return 0
		case less:
	...
	input := utils.ReadLines(fileName)
	return int(calcPacket(parseCode(input[0])))
}
...
func parseCode(code string) packet {
	bitString := hexStringToBitString(code)

	result, _ := parsePacket(bitString)
	return result
}
...
func parsePacket(bits string) (packet, string) {
	...
	result.version = bitStringToInt(bits[:3])
	result.packetType = packetType(bitStringToInt(bits[3:6]))

	switch result.packetType {
	case literal:
		data := ""
		for i := 6; i < len(bits); i += 5 {
			data = data + bits[i+1:i+5]
			if bits[i] == '0' {
				result.value = bitStringToInt(data)
				return result, bits[i+5:]
			}
		}
	default:
		...
		switch lengthType {
		case '0':
			subPacketLength := bitStringToInt(bits[7:22])
			remaining = bits[22+subPacketLength:]
			thisPacketRemaining := bits[22 : 22+subPacketLength]
			for len(thisPacketRemaining) > 0 {
				subPacket, nextSubpacket := parsePacket(thisPacketRemaining)
				subPackets = append(subPackets, subPacket)
				thisPacketRemaining = nextSubpacket
			}
		case '1':
			numberOfPackets := bitStringToInt(bits[7:18])
			remaining = bits[18:]
			var i int64
			for i = 0; i < numberOfPackets; i++ {
				subPacket, nextSubpacket := parsePacket(remaining)
				subPackets = append(subPackets, subPacket)
				remaining = nextSubpacket
			}
		}
...
func bitStringToInt(bitString string) int64 {
	result, err := strconv.ParseInt(bitString, 2, 64)
	if err != nil {
		fmt.Printf("Error: bitString to int failed. %s, %v", bitString, err)
		return -1
	}
	return result
}

func hexStringToBitString(hexString string) string {
	result := ""
	for _, char := range hexString {
		thisChar, err := strconv.ParseInt(string(char), 16, 32)
		if err != nil {
			fmt.Printf("Error: hexString to bitstring failed. %s, %v", hexString, err)
			return ""
		}
		result = fmt.Sprintf("%s%04b", result, thisChar)
	}
	return result
}

func totalVersions(input packet, runningTotal int64) int64 {
	result := runningTotal + input.version

	for _, subPacket := range input.subPackets {
		result += totalVersions(subPacket, runningTotal)
	}
