...
func Part1(fileName string) float64 {
	...
	bits := len(input[0])
	numbers := utils.BinaryParse(input)

	return float64(calculatePower(numbers, bits))
}
...
0 bits (5), so keep only the 7 numbers with a 1 in the first position: 11110, 10110, 10111, 10101, 11100, 10000,
and 11001.
...
func Part2(fileName string) int {
	...
	bits := len(input[0])
	numbers := utils.BinaryParse(input)
	return calculateLifeSupport(numbers, bits)
}

func calculateLifeSupport(input []int, bits int) int {
	oxygenArray := input
	co2Array := input

	for bit := bits - 1; bit >= 0; bit-- {
		oxygenArray = filterByBit(oxygenArray, bit, false)
		co2Array = filterByBit(co2Array, bit, true)
	}

	return oxygenArray[0] * co2Array[0]
}

func calculatePower(input []int, bits int) int {
	gamma := 0
	epsilon := 0
	for bit := 0; bit < bits; bit++ {
		mostBits := countBit(input, bit)
		gamma = gamma | (mostBits << bit)
		epsilon = epsilon | ((mostBits ^ 1) << bit)
	}

	return gamma * epsilon
}

func filterByBit(input []int, bit int, inverse bool) []int {
	var result []int
...
func countBit(input []int, bit int) int {
	zeros := 0
	ones := 0
